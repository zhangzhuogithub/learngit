...
<%@include file="common/head.jsp" %>
	<div>
		<ul class="breadcrumb">
			...
			<li>
				<a href="queryUsers.action">用户管理</a>
			</li>
		</ul>
	</div>
	...
	<div class="box-content">
		<form action="<%=request.getContextPath() %>/user/queryUsers.action" method="post">
			<div class="searcharea">
				...
				<select name="s_roleid" style="width:100px;">
					...
					<c:forEach var="role" items="${roleList}">
						<option <c:if test="${role.id==s_roleid}">selected="selected"</c:if> value="${role.id}">${role.rolename}</option>
					</c:forEach>
				</select>
				是否启用:
			  		...
			  		<c:forEach items="${page.items}" var="user">
			  			<tr>
							...
							<td class="center">
								...
								<c:if test="${user.isstart == 1}"><input type="checkbox" disabled="disabled" checked="checked"/></c:if>
							</td>
							<td class="center">
			...
			<ul>
				<c:choose>
					<c:when test="${page.currentPage == 1}">
						<li class="active">
							  <a href="javascript:void();" title="首页">首页</a>
						</li>
					</c:when>
					<c:otherwise>
						<li>
							<a href="<%=request.getContextPath()%>/user/queryUsers.action?currentPage=1&s_logincode=${s_logincode}&s_refercode=${s_refercode}&s_roleid=${s_roleid}&s_isstart=${s_isstart}" title="首页">首页</a>
						</li>
					</c:otherwise>
				</c:choose>
				<c:if test="${page.prevPages != null}">
					<c:forEach items="${page.prevPages}" var="num">
						<li>
							<a href="<%=request.getContextPath()%>/user/queryUsers.action?currentPage=${num}&s_logincode=${s_logincode}&s_refercode=${s_refercode}&s_roleid=${s_roleid}&s_isstart=${s_isstart}" title="${num}">${num}</a>
						</li>
					</c:forEach>
				</c:if>
				...
				<c:if test="${page.pageCount!=null}">
					...
					<c:when test="${page.currentPage == page.pageCount}">
						<li class="active">
							  <a href="javascript:void();" title="尾页">尾页</a>
						</li>
					</c:when>
					<c:otherwise>
						<li><a href="<%=request.getContextPath()%>/user/queryUsers.action?currentPage=${page.pageCount}&s_logincode=${s_logincode}&s_refercode=${s_refercode}&s_roleid=${s_roleid}&s_isstart=${s_isstart}" title="尾页">尾页</a></li>
					</c:otherwise>
				</c:choose>
				</c:if>
				<c:if test="${page.pageCount==null}">
...
<form action="addUser.action" enctype="multipart/form-data" method="post" onsubmit="return addUserFunction();">
...
<form action="updateUser.action" enctype="multipart/form-data" method="post" onsubmit="return modifyUserFunction();">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal">×</button>
		<h3>修改用户信息</h3>
	</div>
...
<script src="<%=request.getContextPath()%>/js/localjs/userlist.js"></script>	
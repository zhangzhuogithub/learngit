			...
			$.ajax({
				...
				success: function(result){
					if(result != "" && "success" == result){
						tip.css("color","green");
						tip.html("修改密码成功 ，下次登录记得使用新密码哦。^_^(两秒后自动关闭弹窗)");
						//关闭弹窗
						setTimeout(function(){
							$("#closebtn").click();
						},2000)
...
//    $('#reply_createTime').val((d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear());
	...
	for(var i = 0;i<json.length;i++){
		...
		for(var j=0;j<json[i].subMenu.length;j++){
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\""+getRootPath()+"\\"+json[i].subMenu[j].funcurl +"\"><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].subMenu[j].functionname + "</span></a></li>";
		}
		result = result +"</ul></li>";
	}
	$("#menus").append(result);

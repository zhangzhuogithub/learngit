...
$("#m_roleId").change(function(){
	...
	if (sel_role==2) {
		$.post("loadUserTypeList.action",{'s_roleid':sel_role},function(result){
...
$("#m_logincode").blur(function(){
	...
	if (mlc != "") {
		//一部哦岸段
		$.post("logincodeIsExist.action",{'logincode':mlc,'id':$("#m_id").val()},function(result){
			if (result == "repeat") {
				$("#modify_formtip").css("color","red");
				$("#modify_formtip").html("该用户名已存在!");
...
$("#m_email").blur(function(){
	...
	if (flag == false) {
		$("#modify_formtip").css("color","red");
		$("#modify_formtip").html("<li>amail格式不正确!</li>");
		...
		success:function(result){
			if ("failed" == result) {
				...
				if (isstart == '1') {
					...
					$('#v_isstart').append("<option value=\"2\" selected=\"selected\">不启用</option><option value=\"1\" >启用</option>");
				}
		...
		success:function(result){
			if ("failed" == result) {
				...
				if (roleId == '2') {
					...
					$.post("loadUserTypeList.action",{'s_role':roleId},function(r){
						if (r != "") {
				...
				if (sex=="") {
					...
					$("#m_sex").append("<option value=\"女\" selected=\"selected\">女</option><option value=\"男\" >男</option>")
				}
				...
				if (isstart=="1") {
					...
					$("#m_isstart").append("<option value=\"2\" selected=\"selected\">不启用</option><option value=\"1\" >启用</option>")
				}
...
$('.modifyusercancel').click(function(){
	$("#m_idPic").html("");
	$("#m_bankPic").html("");
	$("#m_isstart").html("");
	$("#modify_formtip").html(""); 
...
$("#a_logincode").blur(function(){
	...
	if (alc != "") {
		//一部哦岸段
		$.post("logincodeIsExist.action",{'logincode':alc,'id':'-1'},function(result){
			if (result == "repeat") {
				$("#add_formtip").css("color","red");
				$("#add_formtip").html("该用户名已存在!");
...
$("#a_email").blur(function(){
	...
	if (flag == false) {
		$("#add_formtip").css("color","red");
		$("#add_formtip").html("<li>amail格式不正确!</li>");
...
function addUserFunction(){
	...
	if ($("#a_logincode").val()=="" || $("#a_logincode").val()==null) {
		$("#add_formtip").css("color","red");
	...
	if ($.trim($("#a_email").val()) != "" && $("#a_email").val() !=null && $("#a_email").attr("key") == "1") {
		$("#add_formtip").css("color","red");
...
function delpic(id,closeSpan,uploadBtn,obj,picPath,picText,fileinputId){
	//del
	$.post("delpic.action",{'id':id,'picPath':picPath},function(result){
		if (result=="success") {
			alert("删除成功");
			$('#'+picText).val("");
...
$(".deluser").click(function(){
	...
	var d_id=d.attr('id');
	if (confirm("您确定要删除["+d_logincode+"]这个用户么?")) {
		$.post("deleteUser.action",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':d_usertype},function(result){
			if (result=="success") {
				alert("删除成功!");
				window.location.href="queryUsers.action"
